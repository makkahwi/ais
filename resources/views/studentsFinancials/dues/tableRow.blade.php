            ...
            @can('update', App\Models\studentsFinancials::class)
                <td>
                    <div class='btn-group'>
                        ...
                        <!-- Showing Button-->
                        <button data-toggle="modal" data-target="#show-modal" id="showing" data-sem="{{ $due->sem->title }}, {{ $due->sem->year->title }}" data-sno="{{ $due->studentNo }} {{ $due->student->user->name }}" data-category="{{ $due->category->title }}" data-oriamount="RM{{ $due->category->amount }}" data-discount="@if($due->discount_id){{ $due->discount->title }} @else Non @endif" data-discharge="@if($due->discount_id)@if($due->discount->type == 'Fixed Amount')RM{{ $due->discount->amount }}@elseif ($due->discount->type == 'Percentage'){{ $due->discount->amount }}%@endif @else 0 @endif" data-final="RM{{ $due->finalAmount }}" class='btn btn-info btn-xs'><i class="far fa-eye"></i></button>
                        ...
                        <!-- Editing Button-->
                        <button data-toggle="modal" id="editing" data-target="#edit-modal" id="editing" data-id="{{ $due->id }}" data-sem="{{ $due->sem_id }}" data-sno="{{ $due->studentNo }}" data-category="{{ $due->category_id }}" data-discount="{{ $due->discount_id }}" data-discharge="@if($due->discount_id){{ $due->discount->amount }}@endif" data-final="{{ $due->finalAmount }}" class='btn btn-warning btn-xs'><i class="fa fa-edit"></i></button>
